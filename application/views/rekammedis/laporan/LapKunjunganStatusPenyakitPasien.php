...
<body>
    ...
    <?php if ($datafilter['format'] == '1') { ?>
        ...
        <!-- Untuk Mengisi Status Penyakit  -->
        <?php
        $statuspenyakit_old = '';
        $statuspenyakit[] = '';
        foreach ($jumlah as $row) : ?>
            <?php if ($row->STS_FORMAT == '1') { ?>
                <?php if ($statuspenyakit_old != $row->Detail) { ?>
                    <?php $statuspenyakit[] = $row->Detail ?>
                <?php } ?>
                <?php $statuspenyakit_old = $row->Detail;  ?>
            <?php } ?>
        <?php endforeach ?>
        <?php array_splice($statuspenyakit, 0, 1); ?>
        <?php $statuspenyakit    = array_values(array_unique($statuspenyakit)); ?>
        ...
        <table class="table table-bordered mt-5 border border-dark" id="testTable">
            ...
            <thead>
                <tr style="text-align: center;" class="align-middle">
                    ...
                    <th colspan="<?= ($no * 3) + 1; ?>">Status Penyakit</th>
                    <th colspan="<?= ($no1 * 3) + 1; ?>">Status Pasien</th>
                </tr>

                <tr style="text-align: center;" class="align-middle">
                    <?php foreach ($statuspenyakit as $row) : ?>
                        <th colspan="3"><?= $row; ?></th>
                    <?php endforeach ?>
            ...
            while ($row <= count($ruangan) - 1) { ?>
                <tr>
                    ...
                    <?php if ($no > 0) { ?>
                        <?php
                        $row1 = 0;
                        while ($row1 <= count($statuspenyakit) - 1) { ?>
                            <?php
                            $row2 = 0;
                            $count = 0;
                            while ($row2 <= count($jumlah) - 1) {
                                if (
                                    $jumlah[$row2]->Detail == $statuspenyakit[$row1] and
            ...
            if ($no > 0) {
                ...
                while ($row1 <= count($statuspenyakit) - 1) { ?>
                    ...
                    while ($row2 <= count($jumlah) - 1) {
                        if (
                            $jumlah[$row2]->Detail == $statuspenyakit[$row1]
                        ) {
...
<script>
    ...
    var tableToExcel = (function() {
        ...
        return function(table, name) {
            ...
            link.download = "Laporan Kunjungan Pasien Berdasarkan Status dan Kasus Penyakit Pasien.xls";
