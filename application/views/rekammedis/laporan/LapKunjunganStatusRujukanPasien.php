...
<body>
    ...
    <?php if ($datafilter['format'] == '1') { ?>
        ...
        <!-- Untuk Mengisi Asal Rujukan  -->
        <?php
        $asalrujukan_old = '';
        $asalrujukan[] = '';
        foreach ($jumlah as $row) : ?>
            <?php if ($row->STS_FORMAT == '1') { ?>
                <?php if ($asalrujukan_old != $row->Detail) { ?>
                    <?php $asalrujukan[] = $row->Detail ?>
                <?php } ?>
                <?php $asalrujukan_old = $row->Detail;  ?>
            <?php } ?>
        <?php endforeach ?>
        <?php array_splice($asalrujukan, 0, 1); ?>
        <?php $asalrujukan    = array_values(array_unique($asalrujukan)); ?>
        ...
        <table class="table table-bordered mt-5 border border-dark" id="testTable">
            ...
            while ($row <= count($ruangan) - 1) { ?>
                <tr>
                    ...
                    <?php if ($no > 0) { ?>
                        <?php
                        $row1 = 0;
                        while ($row1 <= count($asalrujukan) - 1) { ?>
                            <?php
                            $row2 = 0;
                            $count = 0;
            ...
            if ($no > 0) {
                ...
                while ($row1 <= count($asalrujukan) - 1) { ?>
                    ...
                    while ($row2 <= count($jumlah) - 1) {
                        if (
                            $jumlah[$row2]->Detail == $asalrujukan[$row1]
                        ) {
...
<script>
    ...
    var tableToExcel = (function() {
        ...
        return function(table, name) {
            ...
            link.download = "Laporan Kunjungan Pasien Berdasarkan Status dan Rujukan Pasien.xls";
