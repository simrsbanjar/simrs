...
class M_KunjunganStatusRujukanPasien extends CI_Model
{
    ...
    function filterbytahun($tahun2, $instalasi)
    {
        ...
        $query = $this->db->query("SELECT * from V_DataKunjunganPasienMasukBsetatusBRujukan where YEAR(TglPendaftaran) = '$tahun2' and KdInstalasi ='" . $instalasi . "'  ORDER BY TglPendaftaran ASC ");

        return $query->result();
    }

    function GetAsalPasienTahun($tahun2, $instalasi)
    {

        $query = $this->db->query("SELECT Judul, Detail  from V_DataKunjunganPasienMasukBsetatusBRujukan where YEAR(TglPendaftaran) = '$tahun2' AND Judul = 'Asal Rujukan' and KdInstalasi ='" . $instalasi . "' GROUP BY Judul, Detail ORDER BY Judul, Detail");
    ...
    function GetStatusPasienTahun($tahun2, $instalasi)
    {
        ...
        $query = $this->db->query("SELECT Judul, Detail  from V_DataKunjunganPasienMasukBsetatusBRujukan where YEAR(TglPendaftaran) = '$tahun2' AND Judul = 'Status Pasien' and KdInstalasi ='" . $instalasi . "' GROUP BY Judul, Detail ORDER BY Judul, Detail");
    ...
    function GetRuanganTahun($tahun2, $instalasi)
    {
        ...
        $query = $this->db->query("SELECT KdRuanganPelayanan, RuanganPelayanan from V_DataKunjunganPasienMasukBsetatusBRujukan where YEAR(TglPendaftaran) = '" . $tahun2 . "' and KdInstalasi ='" . $instalasi . "' GROUP BY KdRuanganPelayanan, RuanganPelayanan");
